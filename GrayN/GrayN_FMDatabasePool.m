...
@interface GrayN_FMDatabasePool()

- (void)pushDatabaseBackInPool:(GrayN_FMDatabase*)db;
- (GrayN_FMDatabase*)db;

@end


@implementation GrayN_FMDatabasePool
...
- (void)pushDatabaseBackInPool:(GrayN_FMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The GrayN_FMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (GrayN_FMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [GrayN_FMDatabase databaseWithPath:_path];
        }
