...
#import "sqlite3.h"
#import "GrayN_FMResultSet.h"
#import "GrayN_FMDatabasePool.h"
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `GrayN_FMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<GrayN_FMResultSet>` - Represents the results of executing a query on an `GrayN_FMDatabase`.
 - `<GrayN_FMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<GrayN_FMDatabasePool>` - A pool of `GrayN_FMDatabase` objects.
 ...
 @warning Do not instantiate a single `GrayN_FMDatabase` object and use it across multiple threads. Instead, use `<GrayN_FMDatabaseQueue>`.

 */

@interface GrayN_FMDatabase : NSObject  {
...
/** Create a `GrayN_FMDatabase` object.
 
 An `GrayN_FMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `GrayN_FMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `GrayN_FMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    GrayN_FMDatabase *db = [GrayN_FMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    GrayN_FMDatabase *db     = [GrayN_FMDatabase databaseWithPath:dbPath];
 ...
 @return `GrayN_FMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `GrayN_FMDatabase` object.
...
/** Execute select statement
 ...
 Executing queries returns an `<GrayN_FMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[GrayN_FMResultSet next]>`) from one record to the other.
 ...
 @return A `<GrayN_FMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see GrayN_FMResultSet
 @see [`GrayN_FMResultSet next`](<[GrayN_FMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (GrayN_FMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (GrayN_FMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (GrayN_FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (GrayN_FMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (GrayN_FMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(GrayN_FMDatabase *adb) {
        ...
        int rowCount = 0;
        GrayN_FMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [GrayN_FMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using GrayN_FMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `FMStatement`, but rather with `<GrayN_FMDatabase>` and `<GrayN_FMResultSet>` only.
