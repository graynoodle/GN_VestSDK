...
/** Represents the results of executing a query on an `<GrayN_FMDatabase>`.
...
@interface GrayN_FMResultSet : NSObject {
    GrayN_FMDatabase          *_parentDB;
...
/** Create result set from `<FMStatement>`
 ...
 @param aDB A `<GrayN_FMDatabase>` to be used
 
 @return A `GrayN_FMResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(FMStatement *)statement usingParentDatabase:(GrayN_FMDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(GrayN_FMDatabase *)newDb;
